...
def store(request):
    ...
    if received_json["FIRST"]:
        ...
        return JsonResponse(
            {
                "time": (29 - a.minute % 30) * 60 + (90 - a.second),
                "message": "Reading will be recorded next time, now time is " + str(a),
            }
        )
    temp = received_json["temp"]
    humid = received_json["humidity"]
    value = Data(temperature=temp, humidity=humid, date=timezone.localtime(value=timezone.now()), sensor=sensor)
    value.save()
...
def graph_per_day(request, pk):
    ...
    return render(
        request, "esp8266/graph_per_day.html", {"form": form, "values": values, "count": len(values), "sensor": sensor}
    )
